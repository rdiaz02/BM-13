...
\title[Intro to the course] {BM-13: Bioinformática Avanzada y Biología de Sistemas, 2014-2015\\ 
Introduction to the course}

% \subtitle
% {Review and reflections} % (optional)

\author[]{Ramón Díaz-Uriarte} %% \\ \Burl{http://ligarto.org/rdiaz}}

\institute[]{Dept. Bioquímica\\
...
% This work is Copyright, \copyright, 2011, Ramón Díaz-Uriarte, and is 
% licensed under the \textbf{Creative Commons }
...
% \node[below=of razon,punkk,yshift=5mm,text width=2cm] (filog) {Evolución/ Filogenia};

 \draw[-] (methods) to (algo);
 \draw[-] (methods) to (stats);
% \draw[-] (razon) to (filog);

 % \node [xshift=-0.86cm,yshift=-7.35cm] at (current page.south west)  {\scriptsize[Sección \ref{D-bbms-objetivos-detalles}, p.\ \pageref{D-bbms-objetivos-detalles}]};
...
%% Para mi es: preparar con antelacion, mucho más expuesto, y corregir
%% deberes A MANO!!!
      \end{itemize}
    \end{itemize}
  \item How?
...
%  \node[left=of criteria,punkk,xshift=5mm,text width=2.4cm] (man)
%  {Documentación: Manuales, Literatura} 
%  edge [->] (criteria); 

%  \node[left=of man,punkk,text width=1.5cm,xshift=5mm] (info) {Búsqueda info}
% edge [<-] (man);
...
El aula cuenta con 20 portátiles macbook con dual boot (win y mac). Si los
...
primer día que vayais a usar portátiles).

B. USO DE VUESTROS PROPIOS PORTATILES

Por otra parte, si quereis, en vez de usar los portátiles del departamento
podeis traer vuestros propios portátiles. En ese caso deberíais:

1. Configurar el acceso a la red inalámbrica de la UAM, si aún no lo
habéis hecho. 

Instrucciones aquí: http://www.uam.es/wifi

2. Tener un navegador (web browser) con los plugins de Java. 

Java se descarga de la página de Sun: www.sun.com. Podeis verificar que
funciona visitando la página 

http://3dsim.bioinfo.cnio.es

y seleccionando uno de los ejemplos. Deberíais ver una proteína, ser
capaces de marcar dominios, etc.

3. Tener Python instalado.

Una versión aceptablemente reciente (>= 2.5) basta. Pero es importante que
esté bastante completa, e incluya en particular el uso de Tcl/Tk. Podeis
verificar esto último siguiendo las instrucciones de Tkinter 
http://wiki.python.org/moin/TkInter

Basta con que llegueis con éxito a la Step 3. Todo lo demás (y qué es
Tkinter) podeis ignorarlo.

4. Tener Pymol instalado. Lo más expeditivo es que useis una de las
