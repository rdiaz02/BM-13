...
bachelor's degree. This is at less technical level than covered in BM-13,
of course, but we deal with issues we will not cover here. If all of this
seems too hard, I might use them to lecture with them on the first day.
...
The UPGMA algorithm is the same UPGMA algorithm you saw in BM-1 for
clustering of microarrays. When reconstructing phylogenies we use data
...
\item With the data from exercise \ref{ex81-ha}, verify the four-point
  condition.  
...
\item Using the data in \ref{bm13-filog1}, reconstruct the phylogeny of
  ...
  \item UPGMA. Is the phylogeny rooted? Are distances ultrametric? When
    ...
    case do not transform using Jukes-Cantor).
